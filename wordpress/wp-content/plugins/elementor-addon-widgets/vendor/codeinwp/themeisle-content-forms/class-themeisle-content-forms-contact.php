...
class ContactForm extends Base {
	...
	public function rest_submit_form( $return, $data, $widget_id, $post_id, $builder ) {
		...
		if ( empty( $data['name'] ) ) {
			$return['msg'] = esc_html__( 'Missing name.', 'elementor-addon-widgets' );
		...
		if ( empty( $data['message'] ) ) {
			$return['msg'] = esc_html__( 'Missing message.', 'elementor-addon-widgets' );
		...
		if ( $result ) {
			...
			$return['msg'] = esc_html__( 'Ops! I cannot send this email!', 'elementor-addon-widgets' );
		}
	...
	private function prepare_body( $body, $data ) {
		...
		<table>
			...
			<tr>
				<td>
					<hr/>
					<?php esc_html_e( 'You recieved this email because your email address is set in the content form settings on ', 'elementor-addon-widgets' ) ?>
					<a href="<?php echo esc_url( get_site_url() ); ?>"><?php bloginfo( 'name' ); ?></a>
				</td>
			</tr>
			</tfoot>
